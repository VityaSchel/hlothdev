...
export default {
  'ru-RU': {
    ...
    'ABOUT_ME': {
      HEADING: 'Привет, меня зовут Виктор, и я являюсь специалистом в области разработки веб-приложений',
      INTRO: dedent`Я профессионально занимаюсь разработкой приложений PWA и инструментов для крупных клиентов более двух лет,
      ...
      BACKEND: dedent`Помимо фронтенда я занимаюсь и бекендом, для своих клиентов создаю сайты «под ключ», настраивая сервер, почту и создавая сам сайт с нуля по желанию заказчика,
      ...
      MY_PROJECTS: dedent`В своих проектах я чаще всего использую веб-серверы <b>Express и Fastify с Nginx</b> (ранее 3 года работал с Apache), базы данных
      ...
      BOTS: dedent`Более четырех лет я разрабатываю <b>ботов для ВКонтакте, Телеграма, Дискорда, Реддита, Яндекс.Алисы</b>, расширения для
      ...
      PARSING: dedent`Также в последнее время я начал увлекаться врапперами API, скрин-скрейпингом и парсингом данных: я создал уже несколько
      ...
      ENDING: dedent`Более подробную информацию о моих навыках вы можете найти в портфолио и в \
        моем <a href='https://github.com/VityaSchel'>аккаунте GitHub</a>, который я веду
  ...
  '_DEFAULT_': {
    ...
    ABOUT_ME: {
      ...
      BACKEND: dedent`In addition to the frontend, I also work with backend. For my clients I create turnkey websites, set up the
        ...
        Figma/Photoshop/Illustrator layout.`,
      MY_PROJECTS: dedent`In my projects, I mostly use
      ...
      PARSING: dedent`Also lately I've started getting into API wrappers, screen-scraping and data parsing: I've already created several
      ...
      WORK_EXPERIENCE_BLOCK: dedent`<ul>
        <li>I have developed several websites for the Russian blogger and streamer (President Aboba, Gadzas of 
            Glad Valakas), which now have hundreds of unique visitors per month</li>
        ...
        <li>I have developed a bot with an admin control panel, installed on a customer's server for the AtmosSeries
            channel in Telegram, which has a reach of tens of thousands of users. It allows you to distribute ads in 
            private chat with bot and counts referral registrations</li>
        <li>Then we continued to work and created a Minecraft server with private survival world and a website with 
            payments processing</li>
        <li>I worked in the team of the National Research Center in Kazakhstan, we were working on CRM for the 
            center's employees, and my job was to create a custom React component</li>
      </ul>`,
      ENDING: dedent`You can find more information on my skills in my portfolio and on 
        my <a href='https://github.com/VityaSchel'>GitHub account</a>, which I've created
